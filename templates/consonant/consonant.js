    ...
    if (!blockNames.some((e) => $blocksWithoutButton.includes(e))) {
      ...
      if ($p) {
        ...
        const buttonsOnly = childNodes.every(($c) => {
          if (isNodeName($c, 'a') || (isNodeName($c, '#text') && re.test($c.textContent))) {
            return true;
          } else if ($c.childNodes.length > 0) {
            return Array.from($c.childNodes).every(($cc) => {
              if (isNodeName($cc, 'a') || (isNodeName($cc, '#text') && re.test($cc.textContent))) {
                return true;
              } else if ($cc.childNodes.length > 0) {
                return Array.from($cc.childNodes).every(($ccc) => isNodeName($ccc, 'a') || (isNodeName($ccc, '#text') && re.test($ccc.textContent)));
              } else return false;
            });
          } else return false;
        });
